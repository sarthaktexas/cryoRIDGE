...
def _draw_sliding_window_panel(ax) -> None:
    ...
    for i, x in enumerate(xs):
        color = ZONE_CAUTION if i == 5 else "#dddddd"
        ec = ZONE_OMIT if i == 5 else "#888888"
    ...
    ax.add_patch(
        Rectangle(
            ...
            1.4,
            fill=False,
            edgecolor=PALETTE_BLUE,
            linewidth=1.0,
            linestyle="--",
        )
    )
    ax.text(5.0, 3.35, "Sliding neighborhood (example: 5 voxels)", ha="center", fontsize=7, color=PALETTE_BLUE)
...
def _draw_comparison_panel(ax) -> None:
    ...
    rows = [
        ...
        ["Gradient energy (zones)", "No (avg only)", "5^3 box on grad^2 of z-scored avg", "Real-space gradient on rho_tilde", "Yes (every voxel)", "Strong"],
    ]
    ...
    for i, row in enumerate(rows):
        ...
        for j, cell in enumerate(row):
            ...
            color = PALETTE_GREEN if cell == "Strong" else ("#666666" if cell == "Weak" else "black")
    ...
    ax.text(
        ...
        "Gradient energy answers placement difficulty — use both axes during building.",
        ha="center",
        va="center",
        fontsize=6,
        wrap=True,
...
def plot_resolvability_metrics_schematic(*, dpi: int = 300) -> plt.Figure:
    """Multi-panel schematic for methods (CC vs local FSC vs local res)."""
    ...
    _metric_card(
        ...
        accent=PALETTE_PURPLE,
        note="Weakly coupled to Q and to gradient energy",
    )
