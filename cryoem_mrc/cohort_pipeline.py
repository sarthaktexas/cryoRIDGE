...
def manifest_rows(
    ...
    *,
    ...
    """Active cohort rows with a local reference map on disk."""
...
def cohort_emdb_ids(manifest: Path | None = None) -> list[str]:
    """EMDB IDs eligible for the reliability cohort (for SLURM array indexing)."""
    return [str(r["emdb_id"]).strip() for r in manifest_rows(manifest or COHORT_MANIFEST)]
...
def _process_row(
    ...
    *,
    ...
    features = features_npz_path(
        data_dir,
        eid,
        feature_thr if density_source == "avg_half" else contour,
    ...
    if metrics_npz.is_file() and not force:
...
def main(argv: list[str] | None = None) -> int:
    ...
    emd_id = None if args.pending else args.emd_id
    rows = manifest_rows(args.manifest, emd_id=emd_id)
    if not rows:
